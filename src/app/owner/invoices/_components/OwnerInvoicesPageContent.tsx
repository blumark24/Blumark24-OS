...
export default function OwnerInvoicesPageContent() {
  ...
  const totalMonthly = subs.filter((s) => s.billingCycleAr === "شهري" && s.isActive).length;
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] space-y-5 lg:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <OwnerPlaceholderCard
          title="إجمالي شهري"
          description="مجموع الاشتراكات الشهرية بناءً على أسعار الباقات."
          icon={Building2}
          accent="blue"
          value={loading ? "…" : totalMonthly > 0 ? `${totalMonthly} اشتراك` : "—"}
          hint="من plan_features × billing_cycle"
        />
        <OwnerPlaceholderCard
      ...
      {/* Subscriptions table as invoice list */}
      <section className="glass-card overflow-hidden">
                ...
                {subs.map((sub, idx) => (
                  <tr key={sub.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 sm:px-5 py-3.5">
                      <span className="text-white font-medium">{sub.orgName}</span>
                    </td>
                    <td className="px-4 sm:px-5 py-3.5">
        ...
        {!loading && subs.length > 0 && (
          <div className="px-5 sm:px-6 py-3 border-t border-white/[0.06] flex items-center justify-between text-[11px] text-[#5f7798]">
            ...
            <span className="flex items-center gap-1.5 border border-[#f59e0b]/30 bg-[#f59e0b]/10 text-[#fbbf24] px-2.5 py-1 rounded-full">
