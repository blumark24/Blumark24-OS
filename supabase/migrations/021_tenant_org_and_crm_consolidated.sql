-- ============================================================
-- 021 — TENANT ORG STRUCTURE + CRM (CONSOLIDATED FOR PRODUCTION)
-- ============================================================
-- Replaces branch migrations 019_tenant_org_structure.sql and
-- 020_tenant_crm_system.sql when those were never applied to production.
--
-- Apply AFTER: 018_organization_manager_rbac.sql
-- Apply BEFORE: 022_* (automation engine) if present
--
-- Creates (IF NOT EXISTS):
--   Org:  departments, teams, positions, employee_relations
--   CRM:  crm_pipeline_stages, crm_deals, crm_client_notes, crm_reminders,
--         crm_contracts, crm_activities, crm_revenue_events
--
-- Note: Frontend does NOT use crm_contacts / crm_companies; clients table
-- remains the company/contact anchor for CRM.
-- ============================================================
...
BEGIN
  ...
  IF to_regclass('public.organizations') IS NULL THEN
    RAISE EXCEPTION '021 requires public.organizations — apply migration 009 first';
  END IF;
END $$;

-- ═══════════════════════════════════════════════════════════════════════════
-- PART A — ORGANIZATION STRUCTURE (ex-019)
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE INDEX IF NOT EXISTS idx_crm_notes_org ON public.crm_client_notes (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_crm_reminders_org ON public.crm_reminders (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_crm_contracts_org ON public.crm_contracts (organization_id);
...
-- ============================================================
-- VALIDATION (run manually in SQL editor after apply)
-- ============================================================
...
--     AND proname IN ('can_manage_tenant_org','can_manage_crm','current_org_id');
--
-- SELECT tablename, policyname FROM pg_policies
--   WHERE schemaname = 'public'
--     AND tablename LIKE 'crm_%' OR tablename IN ('departments','teams','positions','employee_relations')
