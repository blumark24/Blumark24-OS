...
/**
 * Dashboard KPI tile — same premium language as OrgPackagePlanCards:
 * strong glow, gradient ambient, progress rail, sparkline.
 */
export function PremiumKpiCard({
  ...
  return (
    <div
      className={cn(
        "group relative w-full min-h-[158px] sm:min-h-[188px] overflow-hidden rounded-3xl border transition-all duration-300",
        "border-white/[0.10] bg-[#070d20]/92 backdrop-blur-xl",
        theme.glow,
    ...
    >
      ...
      <div
        className="pointer-events-none absolute -top-12 -right-8 h-28 w-28 rounded-full blur-2xl opacity-50"
        style={{ background: "var(--kpi-orb, rgba(34,211,238,0.35))" }}
        aria-hidden
      />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(100%_55%_at_50%_0%,rgba(255,255,255,0.07),transparent_62%)]" />
      ...
      <div className="relative z-10 flex h-full flex-col justify-between gap-3 p-3.5 sm:p-4 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className={cn(WS_ICON_ORB, "w-10 h-10 sm:w-11 sm:h-11", theme.orb)}>
        ...
        <div className="space-y-2 min-w-0">
          ...
          <div className="flex items-end justify-between gap-2">
            <div className={cn("h-1 flex-1 rounded-full overflow-hidden bg-white/[0.08]", theme.accent)}>
              <div
                className={cn("h-full rounded-full transition-all duration-500 bg-gradient-to-l from-current to-transparent opacity-90")}
                style={{ width: progressWidth ?? "42%" }}
              />
            </div>
            <div className="h-5 w-14 shrink-0 opacity-70">
