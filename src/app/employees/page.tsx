...
import { PageHero, KpiStatCard, WorkspaceEmptyInline } from "@/components/ui/workspaceUi";
import { cn } from "@/lib/utils";
import { Users, Plus, Search, Star, Edit2, Trash2, X, Eye, EyeOff, Building2, Briefcase } from "lucide-react";
import {
  usePermissions,
  ROLE_LABELS,
  UserRole,
...
/**
 * Department color tokens — derived from semantic CSS vars so they look right
 * in both dark and light theme automatically.
 */
const deptColors: Record<string, string> = {
  "الإدارة":         "var(--ws-cyan)",
  "الإدارة العليا":  "var(--ws-cyan)",
  "الهجوم":          "var(--ws-rose)",
  "وكالة الهجوم":    "var(--ws-rose)",
  "الإبداع":         "var(--ws-violet)",
  "التصميم":         "var(--ws-emerald)",
  "الحملات":         "var(--ws-amber)",
  "خدمة العملاء":    "var(--ws-sky)",
  "المالية":         "#ff7a3d",
  "العمليات":        "var(--ws-cyan)",
  "وكالة الدفاع":    "var(--ws-violet)",
  "تقنية المعلومات": "var(--ws-violet)",
  "AI Lab":          "var(--ws-violet)",
};
    ...
    <DashboardLayout>
      <div className={WS_PAGE}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 min-w-0">
          ...
          <KpiStatCard label="الموظفون النشطون" value={String(stats.active)} icon={Briefcase}   accent="emerald" showLive={false} showSparkline={false} />
          <KpiStatCard label="الأقسام"          value={String(stats.depts)}  icon={Building2}   accent="sky"     showLive={false} showSparkline={false} />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              className="input-dark pe-9 py-2 text-sm"
          ...
          <div className="flex flex-wrap gap-2">
            {["الكل", ...DEPARTMENTS, ...uniqueDepts.filter((d) => !DEPARTMENTS.includes(d))].map((d) => (
              <button
                ...
                className={cn(
                  "px-3 py-1.5 rounded-xl text-xs font-medium transition-all min-h-[36px] touch-manipulation",
                  deptFilter === d
        ...
        {error && (
          <div
            className={cn(WS_CARD, "p-4 text-sm flex items-center justify-between gap-3")}
            style={{ borderColor: "var(--ws-rose-ring)" }}
            role="alert"
          >
            <span className="text-rose-300">{error}</span>
            <button
              onClick={() => refetch()}
              className="px-3 py-1.5 rounded-xl bg-[var(--ws-rose-soft)] hover:opacity-80 transition-opacity text-xs whitespace-nowrap text-rose-200 touch-manipulation"
            >
              إعادة المحاولة
            </button>
          </div>
        )}
        {loading && <div className={cn("text-center py-8 text-sm", WS_MUTED)}>جارٍ التحميل...</div>}
          ...
          <div className={cn(WS_CARD, "overflow-hidden p-0")}>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[700px]">
                ...
                <tbody>
                  {filtered.map((emp) => {
                    const color = deptColors[emp.department] ?? "var(--ws-cyan)";
                    return (
                      <tr key={emp.id} className="table-row border-b border-[var(--ws-border-subtle)] last:border-0">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-[color:var(--ws-text-primary)] font-medium">{emp.name}</div>
                              <div className={cn("text-xs", WS_MUTED)}>{emp.email}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className={cn("px-4 py-3 text-xs", WS_MUTED)}>{SYS_ROLE_LABELS[emp.role] ?? ROLE_LABELS[emp.role as UserRole] ?? emp.role}</td>
                        <td className="px-4 py-3">
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4, 5].map((s) => (
                              <Star
                                key={s}
                                size={12}
                                fill={s <= (emp.performance ?? 0) ? "var(--ws-amber)" : "none"}
                                className={s <= (emp.performance ?? 0) ? "text-amber-400" : "text-[color:var(--ws-border-strong)]"}
                              />
                            ))}
                          </div>
                        </td>
                        ...
                        <td className={cn("px-4 py-3 text-xs", WS_MUTED)}>{emp.joinDate}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          {isAdmin && (
                            <div className="flex items-center gap-2">
                              <button onClick={() => openEdit(emp)} aria-label="تعديل الموظف" className="p-1.5 rounded-lg text-[color:var(--ws-text-secondary)] hover:text-cyan-300 hover:bg-[var(--ws-cyan-soft)] transition-all">
                                <Edit2 size={14} />
                              </button>
                              <button onClick={() => handleDelete(emp)} aria-label="حذف الموظف" className="p-1.5 rounded-lg text-[color:var(--ws-text-secondary)] hover:text-rose-400 hover:bg-[var(--ws-rose-soft)] transition-all">
                                <Trash2 size={14} />
                              </button>
                            </div>
                          )}
        ...
        >
          <div className={cn(WS_GLASS_MODAL, "max-w-lg")}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={closeModal} className="text-[color:var(--ws-text-secondary)] hover:text-[color:var(--ws-text-primary)] touch-manipulation" aria-label="إغلاق">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className={cn(WS_MUTED, "block text-xs mb-1.5")}>البريد الإلكتروني *</label>
                  <input
              ...
              {!editId && (
                <div>
                  <label className={cn(WS_MUTED, "block text-xs mb-1.5")}>كلمة المرور *</label>
                  <div className="relative">
                    <input
                      className="input-dark text-sm pe-10"
                    ...
                    <button
                      ...
                      className="absolute end-3 top-1/2 -translate-y-1/2 text-[color:var(--ws-text-secondary)] hover:text-[color:var(--ws-text-primary)] touch-manipulation"
                      aria-label={showPass ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}
                    >
                      {showPass ? <EyeOff size={15} /> : <Eye size={15} />}
                    </button>
                  </div>
                  <p className="text-[10px] text-[color:var(--ws-text-tertiary)] mt-1">8 أحرف على الأقل · حرف كبير · حرف صغير · رقم · رمز (!@#$...)</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className={cn(WS_MUTED, "block text-xs mb-1.5")}>الراتب (SAR)</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={cn(WS_MUTED, "block text-xs mb-1.5")}>القسم</label>
                ...
                <div>
                  <label className={cn(WS_MUTED, "block text-xs mb-1.5")}>الدور</label>
