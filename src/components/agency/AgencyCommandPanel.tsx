...
import { Activity, AlertTriangle, Briefcase, CheckCircle2, Users } from "lucide-react";
import {
  lifecycleStages,
  taskStages,
  isTaskDelayed,
...
interface AgencyCommandPanelProps {
  /** Compact header for modal; full header on standalone page. */
  variant?: "page" | "modal";
}

export default function AgencyCommandPanel({ variant = "modal" }: AgencyCommandPanelProps) {
  const {
    employees,
    clientBuckets,
    taskBuckets,
  ...
  return (
    <div className="space-y-5 text-white" dir="rtl">
      <section className="rounded-2xl border border-cyan-400/20 bg-[linear-gradient(135deg,rgba(14,28,52,0.95),rgba(22,49,93,0.85))] p-4 sm:p-6 shadow-[0_0_40px_rgba(34,211,238,0.15)]">
        <h1 className={`${headerClass} font-bold`}>{AGENCY_COMMAND_LABEL}</h1>
        <p className="mt-2 text-sm sm:text-base text-cyan-100/80">
          لوحة قيادة الوكالة — متابعة العملاء والمهام والتنفيذ اليومي
        </p>
        <div className="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={label}
              className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-md min-h-[96px]"
            >
              ...
              <div className="space-y-2 max-h-72 overflow-auto pr-1">
                {(clientBuckets[stage] ?? []).length === 0 ? (
                  ...
                  clientBuckets[stage].map((c) => (
                    <article
                      key={c.id}
                      className="rounded-lg border border-white/10 bg-white/[0.03] p-2.5 text-xs space-y-1"
                    >
                      ...
                      <div className="text-white/70">
                        {c.city || "غير محدد"} • {c.packageType || "غير محدد"}
                      </div>
                      <div className="text-white/80">
                        {typeof c.contractValue === "number"
                          ? `${c.contractValue.toLocaleString()} SAR`
                          : "غير محدد"}
                      </div>
                    </article>
                  ))
        ...
        {employees.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {employees.map((e) => (
              <article
                key={e.id}
                className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-1.5 text-sm"
              >
                ...
                <div className="text-cyan-200">
                  الأداء: {typeof e.performance === "number" ? `${e.performance}%` : "غير محدد"}
                </div>
              </article>
            ))}
          </div>
        ...
        {isLoading ? (
          ...
          <ul className="space-y-2 text-sm">
            {insights.map((line, idx) => (
              <li key={idx} className="rounded-lg border border-white/10 bg-white/[0.03] p-2.5">
                {line}
              </li>
            ))}
          </ul>
